...
def parse_args():
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Output fasta file. If not provided, input file will be used.",
    )
    parser.add_argument(
        "-p",
        "--prefix",
        help="Prefix for renaming. Default is the original fasta name.",
    )
    ...
    with open(sequence_file, "r") as file:
        for record in SeqIO.parse(file, "fasta"):
            ...
            new_prefix = (
                args.prefix
                if args.prefix
                else sequence_file.split("/")[-1].split(".")[0]
            )
