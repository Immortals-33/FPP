...
if __name__ == "__main__":
    ...
    parser.add_argument("--rg", type=float, help="Value of Rg for filtering threshold.")
    parser.add_argument(
        "--root-path",
        type=Path,
    ...
    if args.reference_pdb and args.rg:
        parser.error(
            "Cannot input both -r and --rg simultaneously! Please just choose one of them instead."
        )
    ...
    with open(out_file, "w") as out:
        ...
        if native:
            ...
            logger.info(
                f"The Rg value (all atoms) of native PDB is {Rg_natural_Atoms}, and the Rg value (CA) of native PDB is {Rg_natural_CA}."
            )
