...
parser.add_argument(
    "-d", "--design_path", type=str, help="Path to the folder containing designed PDBs"
)
parser.add_argument(
    "-r", "--reference_pdb", type=str, help="Path to the reference PDB file"
)
parser.add_argument(
    "-p",
    "--polar-score",
...
parser.add_argument(
    "-c", "--calculate", type=str, help="Calculate surface polar score for a single PDB"
)
parser.add_argument(
    "--root-path",
    type=Path,
...
if args.reference_pdb and args.polar_score:
    parser.error(
        "Cannot input both -r and --polar-score simultaneously! Please just choose one of them instead."
    )
...
if __name__ == "__main__":
    ...
    if args.calculate is not None:
        ...
        print(
            f"The polar score of {os.path.basename(os.path.normpath(args.calculate))} is {polorScore}."
        )
        logging.info(
            f"The polar score of {os.path.basename(os.path.normpath(args.calculate))} is {polorScore}."
        )
        ...
        with open(out_file, "w") as out:
            ...
            if native:
                ...
                logger.info(
                    f"The surface polar score of native PDB is {polarScore_natural}."
                )
            for pdb_file in sorted(os.listdir(design_path), key=natural_sort_key):
                if pdb_file.endswith(".pdb"):
                    ...
                    logger.info(
                        f"Sample: {pdb}, Surface Polar Score = {polarScore_designed}"
                    )
