...
parser = argparse.ArgumentParser(
    description="Caculate motif RMSD based on a given set of PDBs and a reference PDB"
)
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to the folder containing the designing PDBs.",
)
parser.add_argument(
    "-r",
    "--reference_pdb",
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to the motif positions of designed PDBs.",
)
parser.add_argument(
    "--motif_r",
    type=str,
    required=True,
    help="Path to the motif positions of the reference PDB.",
)
parser.add_argument(
    "--rmsd", type=float, default=1.0, help="Filtering threshold of Motif-RMSD"
)
parser.add_argument(
    "--plddt", type=float, default=80.0, help="Filtering threshold of pLDDT"
)
parser.add_argument(
    "--root-path",
    type=Path,
...
def motifRMSD(
    ref_pdb, des_pdb, ref_motif_txt, des_motif_txt=None, segment="\t", option="CA"
):
    """
...
def coarse_rmsd(tgt_pdb, ref_pdb, option="bb"):
    ...
    if option == "full":
        ...
        for atom_ref in structure_ref.get_atoms():
            if (
                atom_ref.get_name() == "CA"
                or atom_ref.get_name() == "N"
                or atom_ref.get_name() == "C"
                or atom_ref.get_name() == "O"
            ):
                coord_ref.append(atom_ref.get_coord())
        for atom_tgt in structure_tgt.get_atoms():
            if (
                atom_tgt.get_name() == "CA"
                or atom_tgt.get_name() == "N"
                or atom_tgt.get_name() == "C"
                or atom_tgt.get_name() == "O"
            ):
                coord_tgt.append(atom_tgt.get_coord())
...
if __name__ == "__main__":
    ...
    with open(out_file, "w") as out:
        ...
        logger.info(
            f"Loaded {len(os.listdir(design_path))} designed PDBs from {design_path}"
        )
